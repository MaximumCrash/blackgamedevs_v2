...
json2md.converters.image = ({input, imageAlt}) => {
  if (input === "http://image-link-here.com/image.png") {return ''};

  let imgURL = input; 
  const imageName = imageAlt || input.match(/[ \w-]+\./g).pop().replace('.', '');

  return `![${imageName}](${imgURL})`;
}

json2md.converters.skills = (input) => {
  return `<Skills>\n\n${input.map((skill) => `${skill.trim()}\n\n`)}</Skills>`.replace(/\,/g, '')
}

json2md.converters.location = (input) => {
  if (input === "WRITE YOUR COUNTRY NAME HERE WITHOUT ACRONYMS") {return ''}; 
...
json2md.converters.websites = (input) => {
  ...
  if (Array.isArray(input)) {
    ...
    return Object.keys(input).map(key => {
      ...
      return `<${fragmentKey}>\n\n${_data.map((site) => `[${site[0].trim()}](${site[1] ? site[1].trim(): ''})\n\n`)}</${fragmentKey}>`.replace(/\,/g, '');
    }).map((type) => `${type}\n\n`.replace(/\,/g, '')).join('');
  }
}
...
const entryTransformer = (file) => {
  ...
  const _peopleData = file.map((entry) => Object.keys(entry).filter((n) => entry[n] !== "" && entry[n] !== null).map((field) => {
    const returnObj = {};
...
const transformToFile = () => {
  ...
  peopleData.map((person, index) => {
    let {name} = person.find((n) => Object.keys(n)[0] === 'name') || `person_${index}`;
    let mdxData = json2md(person);

    const fileName = name.trim().replace(/\s/g, '_').replace(/[^a-zA-Z0-9\_]/g, '');
 
    fs.writeFile(`${__dirname}/../directory/${fileName}.mdx`, mdxData, (err, data) => {
  ...
  companyData.map((company, index) => {
    let {name} = company.find((n) => Object.keys(n)[0] === 'name') || `company_${index}`;
    let mdxData = `---\nisCompany: true\n---\n\n${json2md(company)}`;
